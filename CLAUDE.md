...
- `problems/*.md` — Kattis problems (title, url, judge, difficulty, `topic`, `order`). One file per problem. A problem with a video lands in its topic's **first** section and one without in its **last**; `section: <id>` overrides that, and an id the topic doesn't declare fails the build.
  - `hints[]` — up to 3 markdown nudges, gated one at a time in the expanded problem (observation → technique → shape of the answer). A 4th fails the build. They render as native `<details>`, so with JS off all of them open independently — the script in `topics/[slug].astro` only narrows that to one at a time, and its frontier is a **high-water mark**, so collapsing a hint never re-locks the ones it unlocked. Hint text ships in the page source; the feature is deliberate friction, not secrecy.
...
- **Placeholder notes are live on the site.** `problems/walrusweights.md` and `problems/sort.md` have bodies beginning "**Placeholder — replace this.**" — contributor instructions rendering to students in the expanded panel on `/topics/dynamic-programming` and `/topics/java-basics`. Either write real notes or delete the body; an empty body renders no panel at all.
- **Difficulties were not audited against the judge.** They were set by hand before the `/add-problem` skill existed, and at least one had drifted (`walrusweights` said `easy`, Kattis says `medium`). Kattis's own band is the source of truth — `difficulty_easy|medium|hard` in the problem page's HTML.
